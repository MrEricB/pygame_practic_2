...
def draw_shield_bar(surf, x, y, percentage):
    ...
    pygame.draw.rect(surf, WHITE, outline_rect, 2) # 2 is thickness of the outline defualt is 1 but 2 looks better
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.speedx = 0
        self.sheild = 100
        self.shoot_delay = 250 # milisecond wait between shots when holding down shoot button
    ...
    def hide(self):
        ...
        self.hidden = True
        self.hide_timer = pygame.time.get_ticks()
        self.rect.center = (WIDTH /2, HEIGHT + 200) # just move off the screen when hiding it
...
player_mini_img = pygame.transform.scale(player_img, (24,19))
...
while running:
    ...
    for hit in hits:
        score += 75 - hit.radius #this way smaller ones are worth more points
    ...
    hits = pygame.sprite.spritecollide(player, mobs, True, pygame.sprite.collide_circle) # takes sprite and a group who to check, who to check against, bool what hit should be deleted or not; gives back list
    for hit in hits:
